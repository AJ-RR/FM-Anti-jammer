...
%     hop_sequence = [1,2,3,4,5];
    ...
    for i = 1:length(hop_sequence)
        if m_length <= hop_duration
            fm_signal_band = bandpass(fm_signal((i-1)*hop_duration:length(fm_signal)),[hop_sequence(i)*fc - 4000,hop_sequence(i)*fc + 4000],fs);
...
%             demodulated_sig(end - hop_duration + length(fm_signal) : end) = awgn(demodulated_sig(end - hop_duration + length(fm_signal) : end), 50);
        else
            if i == 1
